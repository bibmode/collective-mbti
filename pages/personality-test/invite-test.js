...
const InviteTest = ({ questions }) => {
  ...
  const handleDoneQuiz = async () => {
    //get answers
    const calculatedData = getMBTIAnalysis(choiceMade, "According To Friends");
    ...
    const newResult = {
      ...
      relation,
      choices: choiceMade,
      mbtiResult: calculatedData,
    };

    let response = await fetch("/api/invites/inviteResult", {
      method: "POST",
...
export async function getStaticProps() {
  const data = inviteQuestions.data;
